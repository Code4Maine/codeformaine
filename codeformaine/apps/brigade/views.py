...
class DashboardView(TemplateView):
    template_name = 'brigade/dashboard.html'
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        now = timezone.now()
        context['commit_days'] = commits_since = getattr(settings, 'brigade_COMMITS_SINCE_DAYS', 14)
...
class ProjectJoinView(JsonView, views.LoginRequiredMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.is_authenticated() and not user in project.members.all():
            ...
            fragments['.member-thumbs'] = render_to_string('brigade/_member_list.html', {'members': project.members.all()})
